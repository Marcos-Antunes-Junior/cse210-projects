...
public class Aerobic : Exercises
{
    private string _name;
    private string _description;
    private List<string> _aerobic = new List<string>();

    public Aerobic(string name, string description) : base(name, description)
    {
        _name = name;
    ...
    {
        string filename = "aerobic.txt";
        string[] lines = System.IO.File.ReadAllLines(filename);
        foreach (string line in lines)
        {
            string[] parts = line.Split(".|");
            string firstName = parts[0];
            _aerobic.Add(firstName);
        }
    }
    ...
    public override void SaveInfoFile()
    {
        ...
        {
            for(int i = 0; i < _aerobic.Count; i++)
            {
                outputFile.WriteLine($"{_aerobic[i]}.|");
            }
        }
    }
    ...
    public override void StartExercise(int reps, int sets, int restTime)
    {
        ...
        {
            Console.Clear();
            Console.WriteLine($"Number of exercises: {_aerobic.Count}/{i + 1}");
            Console.WriteLine(_aerobic[i]);
            string userInput;
            Console.WriteLine("");
