...
public class LowerBodyStrength : Exercises
{
    ...
    {
        string filename = "lower_strength.txt";
        string[] lines = System.IO.File.ReadAllLines(filename);
        foreach (string line in lines)
        {
            string[] parts = line.Split(".|");
            string firstName = parts[0];
            _lowerStrength.Add(firstName);
        }
    }
    ...
    public override void SaveInfoFile()
    {
        ...
        {
            for (int i = 0; i < _lowerStrength.Count; i++)
            {
                outputFile.WriteLine($"{_lowerStrength[i]}.|");
            }
        }
    }
    ...
    public override void StartExercise(int reps, int sets, int restTime)
    {
        ...
        {
            Console.Clear();
            Console.WriteLine($"Number of exercises: {_lowerStrength.Count}/{i + 1}");
            Console.WriteLine(_lowerStrength[i]);
            string userInput;
            Console.WriteLine("");
