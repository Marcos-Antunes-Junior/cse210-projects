...
public class UpperBodyDef : Exercises
{
    private string _name;
    private string _description;
    private List<string> _upperDef = new List<string>();

    public UpperBodyDef(string name, string description) : base(name, description)
    {
        _name = name;
    ...
    {
        string filename = "upper_def.txt";
        string[] lines = System.IO.File.ReadAllLines(filename);
        foreach (string line in lines)
        {
            string[] parts = line.Split(".|");
            string firstName = parts[0];
            _upperDef.Add(firstName);
        }
    }
    ...
    public override void SaveInfoFile()
    {
        ...
        {
            for(int i = 0; i < _upperDef.Count; i++)
            {
                outputFile.WriteLine($"{_upperDef[i]}.|");
            }
        }
    }
    ...
    public override void StartExercise(int reps, int sets, int restTime)
    {
        ...
        {
            Console.Clear();
            Console.WriteLine($"Number of exercises: {_upperDef.Count}/{i + 1}");
            Console.WriteLine(_upperDef[i]);
            string userInput;
            Console.WriteLine("");
